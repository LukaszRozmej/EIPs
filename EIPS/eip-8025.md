...
- `hash_seed` is a uniformly random 64-bit value drawn by the host for each
  proof attempt. Guests that key internal containers by payload-controlled
  data — accessed storage cells, touched accounts, code by hash — SHOULD derive
  those containers' hash functions from it; a construction wanting a wider key
  MAY stretch it. It is private prover input and MUST NOT influence the guest
  output.
...
`hash_seed` is available to the guest for seeding the hash functions of its
internal containers. The guest output MUST NOT depend on it: for all other
input held equal, `StatelessValidationResult` MUST be identical for every
`hash_seed` value. Any ordering that reaches the output MUST therefore be
derived by sorting keys, never from the iteration order of a seeded container.
