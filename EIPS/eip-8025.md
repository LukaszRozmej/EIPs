...
Nothing needs distributing. The seed is absent from `PublicInput`, so proofs produced under different seeds share the same public input and remain interchangeable, leaving proof deduplication and `MAX_EXECUTION_PROOFS_PER_PAYLOAD` untouched; and it rides the single `StatelessInput` for one logical execution, so segmented proving inherits it. Sixty-four bits is sized for the guessing bound and stays a single register on the 64-bit RISC-V targets, but the width is not the security parameter — where the seed enters the hash is.

Alternatives considered:

- **Derive the seed from public data**, such as `new_payload_request_root` or `prev_randao`. Rejected: the payload's author knows the payload and anything derived from it, and RANDAO for the slot is known before the payload is built.
