...
- `hash_seed` is a uniformly random 64-bit value drawn by the host for each
  proof attempt. It is private prover input; the guest validation rules below
  govern its use. A construction wanting a wider key MAY stretch it.
...
hash functions from `hash_seed`, and MUST apply it where no choice of keys can
cancel it. A seed folded once into an accumulator that key words are afterwards
combined into drops out of the difference between two keys, leaving collisions
that hold for every seed; such a construction does not satisfy this
requirement. A guest that keys no such container has nothing to seed and
ignores the field.

The guest output MUST NOT depend on `hash_seed`: for all other input held
equal, `StatelessValidationResult` MUST be identical for every `hash_seed`
value. Any ordering that reaches the output MUST therefore be derived by
sorting keys, never from the iteration order of a seeded container.
...
`hash_seed` MUST be drawn independently for each proof attempt from a
cryptographically secure random source. It MUST NOT be derived from the block,
the witness, or any other value available to the payload's author, and MUST NOT
be published alongside the proof.
...
Revision 2 appends `hash_seed` to `SSZStatelessInput`; because that container
has a fixed layout, the added field is a serialization change and takes a new
revision rather than extending revision 1.
...
A guest keys internal containers — accessed storage cells, touched accounts, code by hash — by data the payload's author chooses. Execution clients use non-cryptographic hash functions for these lookups and rely on a per-process random seed so that a colliding key set found against one node does not transfer to another. A guest cannot draw its own randomness: everything it reads must come from its committed input. Absent a seed in that input, every guest of a given implementation and version maps a given key to the same bucket, indefinitely.

That makes a hash-flooding attack cheap to mount and reusable. A storage slot index is an unconstrained 256-bit value chosen by the contract that touches it, and the guest keys its per-block storage containers by exactly that value under a hash function that is public, fixed, and — for the multiply-and-fold constructions these containers typically use — invertible, so keys that land in one bucket can be derived rather than searched for. The work is done once, offline, against the published guest; it needs no access to any prover and does not expire. A payload that then touches `n` such slots costs the guest quadratic container work while costing its author only the ordinary gas for `n` storage operations, and the same key set remains valid against every prover running that guest version. Contract addresses and code hashes give further, more expensive variants of the same shape. The gas limit bounds `n`, so the blow-up is bounded — but a bounded quadratic is still the difference between a payload that proves inside a cycle budget and one that does not.

That cost is paid in proving rather than in consensus. The payload still validates to the same result; what rises is the cycle count needed to prove it, potentially far enough to exhaust a prover's budget so that no proof is produced. Under this EIP that degrades to the fallback already specified — verifiers re-execute — which makes it a proving-availability concern and not a soundness one. It is nonetheless worth denying, because restoring the mitigation the execution layer already depends on costs one field.

Sixty-four bits is sized for the guessing bound rather than for any particular construction: an adversary must produce colliding keys without knowing the seed, and a guest that wants a wider key for a keyed pseudorandom function can stretch it without gaining or losing unpredictability. It also stays a single register on the 64-bit RISC-V targets these guests compile to, which matters because the seed is read on every container lookup. The width is not the security parameter — where the seed enters the hash is, as noted under Security Considerations.

Determinism of a proof is a property of the guest as a function of its input, not of the guest behaving identically across runs. A per-attempt seed therefore keeps the proof reproducible and verifiable while making the key-to-bucket mapping unpredictable to whoever built the payload. Because the seed is private prover input that cannot influence the public result, a prover that exhausts its budget on an unlucky or adversarial seed may simply retry with a fresh one.

`hash_seed` needs no distribution and no coordination, which is most of why it is cheap. A verifier does not need it: the result is invariant under it by construction, so the seed would tell a verifier nothing, and it is deliberately absent from `PublicInput`. Two provers that prove the same payload under different seeds therefore produce identical public inputs and interchangeable proofs, leaving the `(new_payload_request_root, proof_type)` deduplication and the `MAX_EXECUTION_PROOFS_PER_PAYLOAD` accounting untouched. It does not need distributing within a prover either: the seed is a field of the single `StatelessInput` for one logical execution, so segmented or continuation-based proving of that execution inherits it with no further plumbing. The only requirements on it are negative — it must not be published, and it must not be reused.

Alternatives considered:

- **Derive the seed from public data**, such as `new_payload_request_root` or `prev_randao`. Rejected: the payload's author knows the payload, and therefore knows anything derived from it; RANDAO for the relevant slot is likewise known before the payload is built. Neither denies precomputation.
- **Require collision-resistant hashing of container keys.** Sound, but it prices every lookup at a cryptographic hash. Several proof systems expose Keccak as a precompile, which narrows the gap, but the cost is still paid on every payload to defend against an occasional one.
- **Bound every container and evict.** Caps the worst case without touching the input schema, but eviction is observable in execution semantics, and the caps are hard to size against a gas limit that changes.
- **A long-lived per-node seed.** Cheaper to plumb, but a single inference — from timing, or from a prover that reveals it — burns that node until it restarts and offers an attacker a stable target.
...
`StatelessInput` serialized under two differing `hash_seed` values, asserting
byte-identical `StatelessValidationResult` from both.
...
**Guest hash flooding.** A guest that keys containers by payload-controlled data under an unseeded non-cryptographic hash can be driven to superlinear work by a payload built to collide, inflating the cycle count of an otherwise ordinary block until it exceeds a prover's budget. The effect is on proof availability rather than validity: the payload's `StatelessValidationResult` is unchanged, and a missing proof falls back to re-execution as described above. `hash_seed` denies the precomputation that makes such a key set reusable across provers and across time, but only where it is applied as the Specification requires: a seed that any choice of keys can cancel is no defence at all. Keying a pseudorandom function with it does not have that weakness; a cheaper mixer must apply it to each key word rather than to the accumulated result alone.

**Hash-seed invariance.** Because `hash_seed` is private prover input chosen freely by the prover, a guest whose output varied with it would let a prover search for a seed that yields a different accepted result. Implementations MUST keep `StatelessValidationResult` invariant under `hash_seed`; concretely, no ordering derived from a seeded container may reach the output. A prover MUST NOT publish or reuse a seed — it is not part of the proof, and revealing it forfeits the protection for that attempt.
